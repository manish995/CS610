...
//####      OPTIMISED KERNEL          ####################
  ...
  uint64_t* d_C2;
  ErrorCheck(13,cudaMalloc(&d_C2, SIZE * sizeof(uint64_t)));
  ErrorCheck(14,cudaEventCreate(&start));
  ErrorCheck(15,cudaEventCreate(&end));
  ErrorCheck(16,cudaEventRecord(start, 0));
  ErrorCheck(17,cudaMemcpy(d_A, h_A, SIZE * sizeof(uint64_t), cudaMemcpyHostToDevice));
  ErrorCheck(18,cudaMemcpy(d_B, h_B, SIZE * sizeof(uint64_t), cudaMemcpyHostToDevice));
  dim3 TPB2(BLOCK_SIZE,BLOCK_SIZE);
  dim3 NB2((N+TPB2.x-1)/TPB2.x, (N+TPB2.y-1)/TPB2.y);
  kernel2<<<NB2, TPB2>>>(d_A, d_B, d_C2);
  ErrorCheck(19,cudaMemcpy(h_gpu2_C, d_C2, SIZE * sizeof(uint64_t), cudaMemcpyDeviceToHost));
  ErrorCheck(20,cudaEventRecord(end, 0));
  ErrorCheck(21,cudaEventSynchronize(end));
  check_result(h_cpu_C, h_gpu2_C);
  ErrorCheck(22,cudaEventElapsedTime(&kernel_time, start, end));
